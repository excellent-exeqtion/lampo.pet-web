...
export default function BasicDataModule() {
  ...
  const basicDataItems: Utils.Field[] = [
    ...
    { label: "Última vacuna", show: true, value: petData.hasVaccine ? (`${petData.lastVaccineName} (${formatDate(petData.lastVaccineDate)})`) : 'No tiene vacunas' },
    { label: "Castrado", show: true, value: petData.isCastrated ? (`Sí (${formatDate(petData.castrationDate)})`) : 'No' },
    { label: "Antipulgas", show: true, value: petData.hasAntiFlea ? (`Sí (${formatDate(petData.antiFleaDate)})`) : 'No' },
  ...
  const contactItems: Utils.Field[] = [
  ...
  const renderField = (item: Utils.Field) => {
    if(!item.show){
      return <div></div>;
    }
    return (
  ...
  return (
    <main style={{ padding: isMobile ? "4rem 1rem 2rem" : "2rem", fontSize: "0.9rem", marginTop: isMobile ? "3.5rem" : "0" }}>
      ...
      <section style={{ marginBottom: "2rem" }}>
        {Utils.renderTitle(<FaUser />, 'Datos básicos')}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "1rem" }}>
          {basicDataItems.map((item) => renderField(item))}
        </div>
      </section>
      <section>
        {Utils.renderTitle(<FaUser />, 'Datos de contacto')}
